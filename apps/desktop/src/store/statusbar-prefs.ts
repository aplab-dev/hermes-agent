...
// session readouts (running/session timers, context meter, tokens/sec) are
// diagnostics most users don't watch, so they start hidden too and the bar
// stays quiet mid-turn. Cache hit rate and session cost stay on: together they
// are the "is this session cheap" readout, and a low hit rate is the first
// sign a mid-session model switch or a shifting system prompt is wasting money.
